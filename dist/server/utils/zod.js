...
const nameInput = z.string(nameError)
    .trim()
    .refine(val => val.length >= name.minLength && val.length <= name.maxLength, nameError);
...
const emailInput = z.string()
    .trim()
    .refine(val => val.length >= email.minLength && val.length <= email.maxLength && val.match(z.regexes.html5Email), emailError);
// const emailInput =
...
const phoneInput = z.string()
    .trim()
    .regex(phone.regex, phoneError);
const inquiryError = { error: inquiry.errorMessage };
const inquiryInput = z.string()
    .trim()
    .refine(val => val.length >= inquiry.minLength && val.length <= inquiry.maxLength, inquiryError);
